...
alter table post modify column titme varchar(255) not null;
...
-- 제약조건 추가
alter table post add post_pk primary key(id);
alter table post add constratint post_fk foreign key(author_id) references author(id);
...
-- default 옵션
-- 보통 enum타입 및 현재시간(current_timestamp)에서 많이 사용
alter table author modify column name varchar(255) default 'lovelybear';

-- auto_increament : 입력을 안 했을 때 마지막에 입력된 가장 큰 값에서
-- 자동으로 증가된 숫자값을 적용
