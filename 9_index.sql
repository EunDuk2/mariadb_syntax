-- pk, fk, unique 제약조건 추가 시에 해당컬럼에 대해 index페이지 자동생성.
-- index가 만들어지면, 조회성능은 향상. 추가/수정/삭제 성능 하락락
...
-- index를 통해 조회성능향상을 얻으려면,
-- 반드시 where조건에 해당 컬럼에 대한 조건이 있어야 함.
select * from author where email="hong@naver.com";

-- 만약에 where조건에서 2컬럼으로 조회 시에, 1컬럼에만 index가 있다면
-- => 인덱스가 있는 컬럼으로 조회
select * from author where name='hong' and email='hong@naver.com';

-- 만약 where조건에서 2컬럼으로 조회시에, 2컬럼 모두에 각각에 index가 있다면
-- 이 경우 db엔진에서 최적의 알고리즘 실행

-- index는 1컬럼 뿐만 아니라, 2컬럼을 대상으로 1개의 index를 설정하는 것도 가능하다.
-- 이 경우 두 컬럼을 and조건으로 조회해야만 index를 사용
-- 복합 인덱스 생성
...
-- index 테스트 시나리오
-- 아래 프로시저를 통해 수십만건의 데이터 insert후에 index생성 전후에 따라
-- 조회성능 확인
DELIMITER //
...
BEGIN
    ...
    WHILE iteration <= max_iterations DO
        ...
        WHILE i <= iteration * batch_size DO
            SET email = CONCAT('eunduk', i, '@naver.com');
...
-- 2. 데이터를 통해 테스트
-- - 회원 : insert + 주소 insert
-- - 글쓰기 : 사용자1 글쓰기 + author_post(insert) -> 여러 개
